...
with full radio and MAC-layer parameters.

---

## Features

- Periodical uplink ("Hello, World!" + counter) on configurable port
- CSV file generated at startup with timestamped rows
- Captures per-event:
...
- SPI connection to SX1276 via spidev
...
| EXTRA     | Comma-separated key=value pairs                      |

### EXTRA field - TX events

    port, counter, rssi, snr, len, bw, cr, freq, output_power,
    tx_data_rate, tx_data_rate_adr, tx_power, nb_trans, nb_trans_cpt,
    nb_available_tx_channel, tx_duty_cycle_timestamp_ms,
    max_duty_cycle_index, rx1_delay_s

### EXTRA field - DOWNDATA events

    port, freq, rssi (dBm), snr (dB)
...
|------------------|------------------------------|------------------|
| Uplink interval  | main_periodical_uplink.c     | 60 s             |
| Delay after join | main_periodical_uplink.c     | 60 s             |
| LoRaWAN region   | example_options.h            | EU868            |
| Uplink port      | main_periodical_uplink.c     | 101              |
| GPIO pins        | modem_pinout.h               | See source       |
...
2. Every 60 s, it sends an uplink with an incrementing counter
...
Modifications (CSV logging, radio/MAC parameter extraction) by [Olivier](https://github.com/oliver791).
